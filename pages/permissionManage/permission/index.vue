<template>
  <div>
    ...
    <el-table :data="tableData3" border ref="permissionsList" style="width: 100%">
      <el-table-column prop="name" label="权限KEY"></el-table-column>
      <el-table-column prop="display_name" label="权限名称"></el-table-column>
      <el-table-column prop="description" label="权限描述">
        <div slot-scope="scope">{{ scope.row.description ? scope.row.description : '暂无描述' }}</div>
      </el-table-column>
    </el-table>
  </div>
</template>
...
export default {
  ...
  watch: {
    searchName: {
      handler: function() {
        const tableRow = this.$refs.permissionsList.$el.querySelectorAll(
          'tbody tr'
        )
        ...
        for (let i = 0; i < this.tableData3.length; i++) {
          if (
            this.searchName &&
            this.tableData3[i].display_name.indexOf(this.searchName) !== -1
          ) {
